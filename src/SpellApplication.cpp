...
bool RemoveSpecificActiveEffects(const std::vector<RE::FormID>& activeEffectIds) {
    ...
    for (RE::ActiveEffect* activeEffect : *activeEffects) {
        ...
        if (!activeEffect || !activeEffect->effect || !activeEffect->effect->baseEffect) {
            logger::trace("RemoveEffects: Skipping effect with null pointers.");
            continue;
        }
        ...
        if (std::find(activeEffectIds.begin(), activeEffectIds.end(), baseFormID) != activeEffectIds.end()) {
            ...
            logger::debug("Found effect to remove: {:#010x} ({})", baseFormID, mgefName);
            logger::debug("  - Instance Ptr: {}", (void*)activeEffect);
            logger::debug("  - Current Duration: {:.2f}", activeEffect->duration);
            logger::debug("  - Elapsed Time: {:.2f}", activeEffect->elapsedSeconds);
    ...
    if (effectsToRemove.empty()) {
        logger::debug("--- No matching effects found to remove. ---");
        return true;
    }

    logger::debug("--- Attempting to dispel {} effects... ---", effectsToRemove.size());
    int dispelCount = 0;
    for (RE::ActiveEffect* effectToDispel : effectsToRemove) {
        ...
        logger::debug("Dispel(true) called on: {:#010x} ({}) Instance: {}", baseFormID, mgefName,
                        (void*)effectToDispel);
    ...
    logger::debug("--- Dispel calls finished for {} effects. ---", dispelCount);
...
void DispellAllSavedSpellsFromActor(RE::Actor& actor) {
    ...
    for (RE::ActiveEffect* activeEffect : *activeEffects) {
        ...
        if (savedSpells.find(linkedSpellFormId) != savedSpells.end()) {
            logger::info("Found active effect with form ID: {:#010x}. Dispel it.", effectFormID);
...
void ApplyAllSavedSpellsToActor(RE::Actor& actor) {
    ...
    logger::info("-------------------Active Effects on Player on load:-------------------");
    RE::MagicTarget* magicTarget = actor.GetMagicTarget();
    if (!magicTarget) {
    ...
    logger::info("-------------------Active Effects on Player on load complete.-------------------");
    ...
    for (RE::ActiveEffect* activeEffect : *activeEffects) {
        ...
        if (AllSavedSpells.find(linkedSpellFormId) != AllSavedSpells.end()) {
            logger::info("Found active effect with form ID: {:#010x}. Resetting duration.", effectFormID);
    ...
    if (appliedSpellsIDs.size() != AllSavedSpells.size()) {
        logger::info("Not all saved spells were applied to the player. {} out of {} spells applied.",
        ...
        for (const auto& formId : notAppliedSpellsIDs) {
            logger::info("Spell ({:#010x}) was not applied to the player. Removing from save.", formId);
            SpellDataPersistence::RemoveSpellFromSave(formId);  // Remove the spell from the save
        }
    } else {
        logger::info("All saved spells were applied to the player.");
    }
...
void LogActiveEffectDetails(RE::ActiveEffect* activeEffect) {
    ...
    logger::info("  -> Applied Effect Found:");
    logger::info("      Name: {}", mgefName);
    logger::info("      MGEF ID: {:#010x}", mgef->GetFormID());
    logger::info("      Spell Duration: {}", spellEffectEntry->effectItem.duration);
    logger::info("      Spell Magnitude: {}", spellEffectEntry->effectItem.magnitude);
    logger::info("      Spell Area: {}", spellEffectEntry->effectItem.area);
    logger::info("      Active Duration (Remaining): {:.2f}", activeEffect->duration);
    logger::info("      Active Magnitude: {:.2f}", activeEffect->magnitude);
    logger::info("      Elapsed Time: {:.2f}s", activeEffect->elapsedSeconds);
}
...
void HandleSavedSpell(RE::ActiveEffect* activeEffect, const SpellCastInfo& castInfo, const char* spellName) {
    ...
    if (castInfo.alreadyOnPlayer) {
        logger::info("Spell '{}' ({:#010x}) is already on player. Dispel it.", spellName,
                        castInfo.spellItem.GetFormID());
        activeEffect->Dispel(false);  // Remove the effect from the actor
        SpellDataPersistence::RemoveSpellFromSave(castInfo.spellItem.GetFormID());
        logger::info("Spell '{}' ({:#010x}) is no longer saved.", spellName, castInfo.spellItem.GetFormID());
    } else {
        ...
        logger::info("Spell '{}' ({:#010x}) is not on player. Apply it.", spellName, castInfo.spellItem.GetFormID());
    }
}
...
void ConvertToPermanentEffectOnPlayer(SpellCastInfo castInfo) {
    ...
    logger::info("Checking applied effects for spell '{}' ({:#010x}) cast last frame...", spellName,
                    castInfo.spellItem.GetFormID());

    bool isSpellSaved = SpellDataPersistence::IsSpellSaved(castInfo.spellItem.GetFormID());
    if (isSpellSaved) {
        logger::info("Spell '{}' ({:#010x}) is saved.", spellName, castInfo.spellItem.GetFormID());
    }
